...
@register_model
def crossvit_9_dagger_224(pretrained=False, checkpoint_dir = None, **kwargs):
    model = VisionTransformer(img_size=[240, 224],
                              patch_size=[12, 16], embed_dim=[128, 256], depth=[[1, 3, 0], [1, 3, 0], [1, 3, 0]],
                              num_heads=[4, 4], mlp_ratio=[3, 3, 1], qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), multi_conv=True)
    if pretrained and checkpoint_dir is not None:
        checkpoint = torch.load(checkpoint_dir, map_location="cpu")

        # # Lấy đúng state_dict của model
        state_dict = checkpoint["model"]
        # model.load_state_dict(state_dict)

        # state_dict = torch.hub.load_state_dict_from_url("comparision/dataset1/with_crossvit/model_best.pth", map_location='cpu')
        # state_dict = torch.load("comparision/dataset1/with_crossvit/model_best.pth", map_location='cpu')
        model.load_state_dict(state_dict)
    return model
