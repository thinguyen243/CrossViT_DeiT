...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='/home/thynnl/Project/CrossViT/output12',
...
def main(args):
    ...
    model.head.out_features = 4
    model.head = nn.Linear(in_features, 4)
    print("[DEBUG] Replace head with", 4)
    print("[DEBUG] Head after replace:", model.head)
    model.to(device)
    ...
    # optimizer = create_optimizer(args, model)
    print("[DEBUG] Final head:", model_without_ddp.head)
    ...
    if args.mixup > 0.:
        # khi mixup, target sẽ được one-hot, dùng SoftTargetCrossEntropy
        criterion = SoftTargetCrossEntropy()
    elif args.smoothing > 0:
        # khi muốn smoothing nhãn nhưng target vẫn là số nguyên
        criterion = LabelSmoothingCrossEntropy(smoothing=args.smoothing)
    else:
        # bình thường dùng CE mặc định của PyTorch (target là số nguyên)
    ...
    if args.initial_checkpoint:
        print("Loading pretrained backbone (skip head)")
        checkpoint = torch.load(args.initial_checkpoint, map_location='cpu')
        state_dict = checkpoint['model'] if 'model' in checkpoint else checkpoint
        # bỏ head.* (1000 lớp) để giữ nguyên head mới (38 hoặc 4 lớp)
        state_dict = {k: v for k, v in state_dict.items() if not k.startswith("head.")}
        missing, unexpected = model.load_state_dict(state_dict, strict=False)
        print("[DEBUG] Missing keys:", missing)
        print("[DEBUG] Unexpected keys:", unexpected)
        print("[DEBUG] model.head:", model.head)
    ...
    if args.resume:
        ...
        state_dict = checkpoint['model'] if 'model' in checkpoint else checkpoint
        # bỏ toàn bộ tham số classifier head 1000 lớp
        state_dict = {k: v for k, v in state_dict.items() if not k.startswith("head.")}
        missing, unexpected = model.load_state_dict(state_dict, strict=False)
        print("[DEBUG] missing keys:", missing)
        print("[DEBUG] unexpected keys:", unexpected)
